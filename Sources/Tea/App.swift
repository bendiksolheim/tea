...
func runApp<Model: Equatable, Message, Meta>(
    ...
    var viewModel = measure("Initial render") { app.render(model, terminal.terminalSize()).layout(terminal) }
            ...
            while polling {
                if let event = terminal.poll() {
                    ...
                    case let .Key(key):
                        ...
                        } else if let node = viewModel.view.contentAt(y: terminal.cursor.y) {
                    ...
                    case let .Resize(size):
                        viewModel = measure("Resize render") { app.render(model, terminal.terminalSize()).layout(terminal) }
                        async {
                            render(viewModel, terminal)
    ...
    messageConsumer.observeValues { message in
        ...
        if modelChanged {
            viewModel = measure("Msg render") { app.render(model, terminal.terminalSize()).layout(terminal) }
            async {
                render(viewModel, terminal)
            }
        }
        ...
        case let .Process(process):
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(100)) {
                ...
                async {
                    ...
                    viewModel = measure("Process render") { app.render(model, terminal.terminalSize()).layout(terminal) }
                    render(viewModel, terminal)
                    startEventPolling()
                }
